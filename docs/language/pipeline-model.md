...
All available operators are listed on the [reference page](operators).
...
of the switch hits an [aggregator](aggregates)), in which case it is typically more performant
...
"foo"
```
But we can make the sort case-insensitive by applying a [function](functions) to the
