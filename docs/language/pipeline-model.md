...
All available operators are listed on the [reference page](operators/_index).

## Pipeline Sources

In addition to the data sources specified as files on the `zq` command line,
a source may also be specified with the [`from` operator](operators/from).

When running on the command-line, `from` may refer to a file, an HTTP
endpoint, or an [S3](../integrations/amazon-s3) URI.  When running in a [SuperDB data lake](../commands/super-db), `from` typically
...
For more detail, see the reference page of the [`from` operator](operators/from),
...
in the [Super JSON format](../formats/jsup)
...
Some operators, like
[`summarize`](operators/summarize) or [`sort`](operators/sort),
...
The [`search`](operators/search) and [`where`](operators/where)
operators "find" values in their input and drop
the ones that do not match what is being looked for.

The [`yield` operator](operators/yield) emits one or more output values
...
The [`fork` operator](operators/fork) copies its input to parallel
branches of a pipeline.  The output of these parallel branches can be combined
in a number of ways:
* merged in sorted order using the [`merge` operator](operators/merge),
* joined using the [`join` operator](operators/join), or
* combined in an undefined order using the implied [`combine` operator](operators/combine).

A pipeline can also be split to multiple branches using the
[`switch` operator](operators/switch), in which data is routed to only one
...
a [`merge` operator](operators/merge)
...
of the switch hits an [aggregator](aggregates/_index)), in which case it is typically more performant
...
"foo"
```
But we can make the sort case-insensitive by applying a [function](functions/_index) to the
...
adding or changing the fields of a record using the [`put` operator](operators/put)
or extracting a subset of fields using the [`cut` operator](operators/cut).
...
the [`summarize` operator](operators/summarize)
```
...
a search expression but is a valid [expression](expressions),
...
The [`put` operator](operators/put) provides this mechanism and the `put`
