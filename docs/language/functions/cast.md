...
The _cast_ function performs type casts but handles both [primitive types](../../formats/zed#1-primitive-types) and
[complex types](../../formats/zed#2-complex-types).  If the input type `t` is a primitive type, then the result
is equivalent to
```
t(val)
```
...
a new [named type](../data-types#named-types) where the name for the type is given by `name` and its
